...
\begin{enumerate}[nosep]
  \item \textbf{Outlier channel ratio} in Layer~0/final layer: Models with ratio~$>$~4$\times$ tend to fail. Appendix~\ref{sec:kv_asymmetry} refines this by separating $K$ and $V$ outliers across 11 models: only models with $K_{\max} \gtrsim 15\times$ \emph{at Layer~0} are catastrophic. $V$ outliers are tolerated at magnitudes up to at least 39.63$\times$.
...
A detailed study of K and V outlier asymmetry across 11 models from six vendors (Appendix~\ref{sec:kv_asymmetry}) refines the picture of when naive INT4 breaks: the catastrophic pattern is specifically $K_{\max} \gtrsim 15\times$ concentrated at Layer~0, which in our sample occurs in 1 of 11 models (Qwen2-7B). $V$ outliers up to 39.63$\times$ and late-layer $K$ outliers up to 10.65$\times$ are tolerated. Because this specific pathology is hard to predict \emph{a~priori} from publicly available model information, the practical recommendation is to apply nsep+pchan4 as a default: negligible cost on the 10/11 models that do not need it, and the difference between a broken and working system on the 1/11 that does.
...
\begin{tabular}{lcccc}
\toprule
Model & $K_{\max}$ & $V_{\max}$ & Layer~0 $K$ & NIAH \\
\midrule
Qwen2-7B \textbf{(pathological)} & \textbf{17.23}$_{L0}$ & 6.09$_{L27}$ & \textbf{17.23} & \textbf{0/26} \\
\midrule
Qwen2.5-14B & 10.65$_{L30/48}$ & 7.71$_{L47}$ & --- & 26/26 \\
Mistral-7B & 6.17$_{L29}$ & 16.47$_{L0}$ & --- & 15/15 \\
Gemma~4 E2B-it & 5.56$_{L9}$ & 7.48$_{L9}$ & --- & 16/16 \\
Gemma~4 26B-A4B (MoE) & 6.82$_{L1}$ & 10.22$_{L22}$ & --- & 21/21 \\
Gemma~4 31B-it & 8.92$_{L53}$ & \textbf{39.63}$_{L12}$ & 6.59 & 21/21 \\
Phi-3-mini-4k-instruct & 5.85$_{L6}$ & 24.37$_{L1}$ & 4.33 & safe (pred) \\
Phi-3-medium-4k-instruct & 5.67$_{L7}$ & 34.99$_{L0}$ & 2.82 & safe (pred) \\
DeepSeek-LLM-7B-Chat & 9.07$_{L6}$ & 10.04$_{L1}$ & 7.45 & safe (pred) \\
Llama-3.1-8B-Instruct & 8.81$_{L29}$ & 4.32$_{L0}$ & 3.09 & safe (pred) \\
Llama-3.2-3B-Instruct & 6.89$_{L25}$ & 4.74$_{L0}$ & 5.84 & safe (pred) \\
\bottomrule
\end{tabular}
\caption{K and V outlier ratios across 11 models. Subscripts indicate the layer at which the maximum is attained. Only Qwen2-7B exhibits the combined pattern of $K_{\max} \ge 15\times$ \emph{at Layer~0}; it is also the only model in our sample that fails multi-needle retrieval under naive INT4. V outliers up to 39.63$\times$ (Gemma~4 31B Layer~12) and late-layer K outliers up to 10.65$\times$ (Qwen2.5-14B Layer~30) are tolerated.}
\label{tab:kv_asymmetry}
\end{table}

\subsection{Refined Characterization of the Pathology}

Three observations refine the simple ``8$\times$ outlier threshold'' of the main text.

\textbf{(1) V outliers are tolerated at magnitudes up to at least 39.63$\times$.} Mistral-7B (V = 16.47 at Layer~0), Phi-3-mini (24.37 at Layer~1), Phi-3-medium (34.99 at Layer~0), and Gemma~4 31B (39.63 at Layer~12) all pass multi-needle retrieval under naive INT4 despite V outlier ratios far exceeding the nominal 8$\times$ threshold. This places an empirical lower bound of roughly 40$\times$ on V outlier tolerance.

\textbf{(2) Late-layer K outliers are tolerated up to at least 10.65$\times$.} Qwen2.5-14B has $K_{\max} = 10.65$ at Layer~30 of 48, and achieves 26/26 multi-needle retrieval under naive INT4. The comparable numerical value at Layer~0 (as in Qwen2-7B) would be catastrophic. Late-layer K errors propagate through fewer subsequent layers, limiting their impact.

\textbf{(3) The pathological pattern is specifically $K_{\max} \gtrsim 15\times$ concentrated at Layer~0.} Qwen2-7B is the only model in our 11-model sample to exhibit this pattern, and it is the only catastrophic failure we have observed across all PPL and NIAH experiments in this paper. The 15$\times$ number is approximate; what matters mechanistically is that a single channel completely dominates the Layer~0 $K$ scale, and Layer~0 errors propagate multiplicatively through every subsequent attention layer.
...
\subsection{Practical Implication: Universal Safety Net}

Because the specific pathology (Layer~0 $K$ with extreme magnitude) is difficult to predict from public model cards and varies substantially across training recipes, a practitioner cannot easily anticipate whether a given model will fail under naive INT4. Our 11-model sample yields a pathology rate of 1/11 ($\approx$9\%), with the single catastrophic case producing 0/26 needle retrieval.

The practical recommendation is therefore to apply nsep+pchan4 \emph{by default}. Our results throughout the paper establish that the method introduces no catastrophic degradation on safe models (worst case: +0.24 $\Delta$PPL, Table~\ref{tab:sweep}); for the small fraction of models harbouring the pathology, it is the difference between a broken production system and a working one.
