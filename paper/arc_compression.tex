...
\author{Kentaro Sato\\info@metaclan.jp\\[4pt]{\small DOI: \href{https://doi.org/10.5281/zenodo.19563193}{10.5281/zenodo.19563193}}\\[2pt]{\small \textcopyright{} 2026 Kentaro Sato. Paper: \href{https://creativecommons.org/licenses/by/4.0/}{CC BY 4.0}. Code: Apache 2.0.}}
\date{}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Never hurts}: Worst-case degradation of +0.24 $\Delta$PPL (on OPT-125m)
...
We identify an additional, independent source of quantization difficulty: \textbf{token-wise norm variation}. In Pre-LN Transformers, hidden-state norms vary significantly across tokens---the L2 norm of KV vectors can differ by 2--5$\times$ within a single sequence. This is a well-documented geometric property: the first principal component of mid-layer hidden states correlates near-perfectly ($|r| > 0.96$) with the token-wise norm \citep{sato2026arc}.
...
The entire method adds only 4 lines to an existing quantization pipeline. Figure~\ref{lst:code} shows a complete PyTorch implementation.
...
\end{lstlisting}
\caption{Complete nsep+pchan4 implementation in PyTorch. The method adds 4 lines (norm separation and re-normalization) to standard per-channel INT4 quantization. No training, calibration, or model-specific tuning is required.}
\label{lst:code}
\end{figure}
...
\textbf{Layer~0 alone} causes nearly all the degradation. Examining the KV cache distribution: Layer~0 absmax~=~166.8 (mean channel absmax: 19.4, outlier ratio: 8.6$\times$), while Layers~4--26 have absmax~9--22 (ratio~$<$~2$\times$). Eight specific channels (indices 61--63, 123--127) contain values 8$\times$ larger than average. Under per-row absmax with $q_{\max}=7$ (INT4), the scale becomes $s=167/7=23.8$, and mid-range values (5--20) are mapped to noisy quantization levels that corrupt attention patterns.
...
\textbf{Negative $\Delta$PPL}: Pythia-2.8B achieves $\Delta$PPL~=~$-$0.06, slightly \emph{outperforming} the uncompressed baseline. We attribute this to per-channel quantization acting as a form of mild regularization.
...
\bottomrule
\end{tabular}
\caption{WikiText-2 evaluation across 7 models (124M to 14B). nsep+pchan achieves up to 1885$\times$ improvement on Qwen2-7B, with the largest gains precisely where naive INT4 fails most severely.}
\label{tab:wikitext}
\end{table}
...
On Mistral-7B (where naive INT4 already works), nsep+pchan4 performs identically---$|\Delta\text{PPL}| < 0.15$ for both methods at all lengths. This confirms the safety-net property: nsep+pchan never hurts on models that don't need it.

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figure2_wikitext2.pdf}
  \caption{WikiText-2 evaluation across 7 models (124M to 14B). (a)~$\Delta$PPL comparison on log scale: naive INT4 (red) fails catastrophically on Pythia and Qwen2-7B, while nsep+pchan (green) keeps $\Delta$PPL below~+4.0 across all models. (b)~Improvement factor: nsep+pchan provides up to 1885$\times$ improvement on Qwen2-7B, with the largest gains precisely where naive INT4 fails most severely.}
  \label{fig:wikitext}
\end{figure}
...
\begin{enumerate}[nosep]
  \item \textbf{Short-context evaluation}: Our primary evaluation uses sequences under 250 tokens. WikiText-2 evaluation on 7 models confirms the pattern on a standard benchmark (\S\ref{tab:wikitext}), but longer-context evaluation (1k--4k tokens) across all models would further strengthen the results.
...
The observation that Pre-LN Transformer hidden states concentrate on a norm-dominant subspace \citep{sato2026arc} provides the theoretical motivation for norm separation. \citet{ethayarajh2019contextual} documented increasing anisotropy in contextualized word representations. \citet{gao2019representation} formalized the representation degeneration problem. \citet{timkey2021bark} showed that ``rogue dimensions'' obscure representational quality. Our work provides the first practical application of these geometric observations to KV cache compression.
...
Combined with per-channel quantization to handle activation outlier channels, the method (nsep+pchan) provides a universal safety net:
\begin{itemize}[nosep]
