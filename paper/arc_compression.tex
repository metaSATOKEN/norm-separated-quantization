...
A study of K and V outlier patterns across 11 additional models from six vendors, plus a 4-model hunt for further catastrophic cases (Appendix~\ref{sec:kv_asymmetry}), motivates and causally tests a pathology signature. Across the combined 27-model sample (12 main sweep + 11 K/V study + 4 hunt screen), the only catastrophic case (Qwen2-7B) is also the only model with $K_{\max} \gtrsim 15\times$ localized at Layer~0. $V$ outliers up to 39.63$\times$ and late-layer $K$ outliers up to 10.65$\times$ were tolerated. GPT-J-6B ($K_{\max}=18.15\times$ at Layer~1, Layer~0 K only 9.31$\times$) provides a natural control showing that K magnitude alone is not sufficient. A selective-compression experiment on Qwen2-7B (Section~H.5) isolates the cause: applying nsep+pchan4 to Layer~0 alone rescues 99.97\% of the catastrophic $\Delta$PPL, while applying naive4 to Layer~0 alone---with all other layers on nsep+pchan4---reproduces the catastrophe. This transforms the Layer~0 claim from correlational (n=1 observational) to causally demonstrated via intervention, even if the prevalence of models harboring the signature remains unknown. Because we cannot audit arbitrary deployed models for this signature a priori, the practical recommendation is to apply nsep+pchan4 as a default, or as a targeted intervention on Layer~0 only for production environments that prioritize throughput on the remaining 27 layers (Section~H.5).
...
\textbf{Practical corollary.} Applying nsep+pchan4 only to Layer~0 (27-of-28 layers remain under naive INT4) is indistinguishable in quality from applying it uniformly, within 0.33 $\Delta$PPL. For production deployments prioritizing throughput, this ``Layer-0-only'' policy preserves 27/28 of the per-layer compute saving of naive INT4 while retaining the quality of nsep+pchan4. The metadata overhead is $O(T)$ FP16 norm scalars on a single layer, a negligible fraction of the KV cache.
