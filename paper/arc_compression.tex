...
\author{Kentaro Sato\\info@metaclan.jp}
\date{}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Post-LN untested}: GPT-1 (Post-LN) was excluded because it does not expose a KV cache through standard APIs. The norm-dominant geometry is weaker in Post-LN models, so benefits may be smaller.
\end{enumerate}
...
The technique is simple enough to implement in a few lines of code and can be combined with existing KV cache compression methods as a preprocessing step.
