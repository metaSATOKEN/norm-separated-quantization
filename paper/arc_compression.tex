...
\begin{enumerate}[nosep]
  \item \textbf{Statistical power}: Our characterization of the catastrophic failure condition rests on a single such case (Qwen2-7B) across 23 models evaluated (12 main + 11 K/V study). The $K_{\max} \gtrsim 15\times$-at-Layer-0 description should therefore be read as a candidate pathology signature observed in our sample, not an established necessary condition. Rigorous threshold characterization requires a larger catastrophic-case sample, which current open-model availability does not provide.
...
A study of K and V outlier patterns across 11 additional models from six vendors (Appendix~\ref{sec:kv_asymmetry}) suggests a candidate pathology signature: the sole catastrophic case in our combined 23-model sample (Qwen2-7B) is also the only model with $K_{\max} \gtrsim 15\times$ concentrated at Layer~0. $V$ outliers up to 39.63$\times$ and late-layer $K$ outliers up to 10.65$\times$ were tolerated without NIAH degradation. The small sample size (one catastrophic case) precludes estimating population failure rate; we present this signature as a candidate hypothesis rather than an established threshold. Because the pathology is hard to predict from publicly available model information, a practical recommendation is to apply nsep+pchan4 as a default: the observed cost on safe models is small, while for the small fraction of models with the pathology it can be the difference between a broken and working system.
...
Three observations refine the simple ``8$\times$ outlier threshold'' of the main text. These are patterns observed in our sample; a single catastrophic case limits the strength of conclusions that can be drawn.

\textbf{(1) V outliers up to at least 39.63$\times$ were tolerated in our sample.} Mistral-7B (V = 16.47 at Layer~0), Phi-3-mini (24.37 at Layer~1), Phi-3-medium (34.99 at Layer~0), and Gemma~4 31B (39.63 at Layer~12) all pass multi-needle retrieval under naive INT4 despite V outlier ratios far exceeding the nominal 8$\times$ threshold. This places an empirical lower bound on V outlier tolerance in our sample, but does not establish an upper bound: extreme cases beyond 39.63$\times$ may or may not behave similarly.

\textbf{(2) Late-layer K outliers up to at least 10.65$\times$ were tolerated.} Qwen2.5-14B has $K_{\max} = 10.65$ at Layer~30 of 48, and achieves 26/26 multi-needle retrieval under naive INT4. In contrast, the comparable numerical value at Layer~0 (as in Qwen2-7B's 17.23$\times$) coincides with catastrophic failure. A plausible mechanistic explanation---that late-layer K errors propagate through fewer subsequent attentions---is developed in Section~H.4, but our one-case sample cannot rule out alternative explanations.

\textbf{(3) Candidate pathology signature: $K_{\max}$ approaching or exceeding roughly 15$\times$ at Layer~0.} Qwen2-7B is the only model in our 11-model K/V sample (and across all 23 models evaluated in the paper) to exhibit this combination, and it is the only catastrophic failure we have observed. We emphasize that a single catastrophic case is statistically insufficient to establish either the 15$\times$ magnitude or the Layer-0 location as a necessary condition; the pattern should be read as a candidate signature motivating future investigation with larger catastrophic-case samples. The mechanistic hypothesis behind the signature---a single channel dominating the Layer~0 $K$ scale, with errors propagating through every subsequent attention---is consistent with the observation but awaits validation with additional samples.
...
The candidate pathology (extreme Layer~0 $K$) is difficult to predict from public model cards, and the training-recipe variation we observe across vendors suggests that whether a newly released model is affected is hard to anticipate. In our 11-model K/V sample we observed 1 catastrophic case (Qwen2-7B); across all 23 models evaluated in the paper (12 main sweep + 11 K/V study) we observed the same single case. This sample is too small to reliably estimate a population failure rate---the observation is compatible with anything from a rare legacy-recipe artifact to a broader pattern that happens to be under-represented in open models.

The practical recommendation that follows is pragmatic rather than statistical: when deploying KV cache INT4 quantization on a Pre-LN Transformer whose outlier profile has not been audited, applying nsep+pchan as a preprocessing step is cheap (minimal memory overhead; implementation-level latency overhead amenable to kernel fusion) and, within our evaluation, did not produce catastrophic degradation on any safe model (worst case: +0.24 $\Delta$PPL, Table~\ref{tab:sweep}). On models that do exhibit the pathological pattern, it recovered NIAH performance from 0/26 to 26/26. We present this as a defensive default rather than a universal cure: the evidence for recovery rests on one observed catastrophic case, and confidence in the default for models with very different training recipes than those tested would grow with additional catastrophic samples.
